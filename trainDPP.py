...
def training(
    dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, 
    checkpoint_iterations, checkpoint, wandb=None, logger=None,
    device=None,  # 新增参数
    is_main_process=False,  # 新增参数
    local_rank=-1  # 新增参数
):
    first_iter = 0
    # 仅主进程初始化TensorBoard
    tb_writer = prepare_output_and_logger(dataset) if is_main_process else None

    # 1. 创建模型并包装为DDP
    modules = __import__('scene')
    model_config = dataset.model_config
    # 初始化模型并移动到当前设备
    gaussians = getattr(modules, model_config['name'])(**model_config['kwargs']).to(device)
    # 若启用DDP，包装模型（仅参数和梯度同步，保持原接口）
    if local_rank != -1:
        gaussians = DDP(
            gaussians,
            device_ids=[local_rank],
            output_device=local_rank,
            find_unused_parameters=True  # 适配可能存在的未使用参数
        )

    # 2. 场景初始化与数据分片（核心：各进程处理不同相机）
    scene = Scene(dataset, gaussians.module if local_rank != -1 else gaussians, 
                 shuffle=False, logger=logger, weed_ratio=pipe.weed_ratio)
    # 获取所有训练相机并按进程分片
    all_train_cams = scene.getTrainCameras()
    if local_rank != -1:
        # 按进程rank划分相机（每个进程处理不重叠的子集）
        rank = dist.get_rank()
        world_size = dist.get_world_size()
        all_train_cams = all_train_cams[rank::world_size]  # 分片逻辑：rank=0取0,world_size,2*world_size...

    # 3. 初始化相机栈（使用分片后的相机列表）
    if pipe.camera_balance:
        aerial_viewpoint_stack = [cam for cam in all_train_cams if cam.image_type == "aerial"]
        street_viewpoint_stack = [cam for cam in all_train_cams if cam.image_type == "street"]
    else:
        viewpoint_stack = all_train_cams.copy()

    # 4. 优化器设置（仅主进程加载checkpoint）
    gaussians.training_setup(opt)
    if checkpoint and is_main_process:
        (model_params, first_iter) = torch.load(checkpoint)
        # 若为DDP模型，需加载到.module
        if local_rank != -1:
            gaussians.module.restore(model_params, opt)
        else:
            gaussians.restore(model_params, opt)

    # 5. 训练循环（保持原有逻辑，仅修改主进程相关操作）
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        gt_image = viewpoint_cam.original_image.to(device)  # 使用当前设备
        ...
        with torch.no_grad():
            ...
            if is_main_process:
                ...
                if iteration % 10 == 0:
                    ...
                    anchor_prim = len(gaussians.module.get_anchor) if local_rank != -1 else len(gaussians.get_anchor)
            ...
            # 训练报告（仅主进程写TensorBoard）
            if is_main_process:
                training_report(
                    tb_writer, dataset_name, iteration, Ll1, loss, l1_loss,
                    iter_start.elapsed_time(iter_end), testing_iterations,
                    scene, getattr(modules, 'render'), (pipe, scene.background),
                    wandb, logger
                )
            
            # 模型保存（仅主进程执行，避免多进程冲突）
            if is_main_process and (iteration in saving_iterations):
                logger.info("\n[ITER {}] Saving Gaussians".format(iteration))
                # 若为DDP模型，保存.module
                scene.save(iteration, model=gaussians.module if local_rank != -1 else gaussians)

            # 可视化（仅主进程执行）
            if is_main_process and (iteration % pipe.vis_step == 0 or iteration == 1):
                # （省略可视化代码，保持不变）
            
            #  densification（密度优化，所有进程同步执行）
            ...
            if is_main_process and (iteration in checkpoint_iterations):
                ...
                model_to_save = gaussians.module if local_rank != -1 else gaussians
                torch.save(
                    (model_to_save.capture(), iteration),
...
if __name__ == "__main__":
    ...
    if args.local_rank != -1:
        # 初始化分布式进程组（NCCL后端适合GPU通信）
        dist.init_process_group(
            backend='nccl',
            init_method='env://',  # 由torchrun自动设置环境变量
    ...
    if is_main_process:
        ...
        logger = None  # 非主进程不输出日志

    # 5. 处理测试/保存迭代参数（仅主进程打印）
    if is_main_process:
    ...
    # 6. GPU设备设置（覆盖原GPU参数，由DDP管理）
    if args.gpu != '-1' and is_main_process:
        logger.info(f'忽略--gpu参数，DDP自动管理设备：{args.local_rank}')
    ...
    torch.manual_seed(42 + dist.get_rank() if args.local_rank != -1 else 42)
    ...
    # 调用training函数时传入DDP相关参数
    training(
        lp, op, pp, exp_name, 
        args.test_iterations, args.save_iterations, args.checkpoint_iterations, 
        args.start_checkpoint, wandb_run, logger,
        device=device,  # 新增：当前进程的设备
        is_main_process=is_main_process,  # 新增：是否为主进程
        local_rank=args.local_rank  # 新增：本地进程编号
    )

    # 11. 训练结束后清理（仅主进程执行后续步骤）
    if is_main_process:
        logger.info("\nTraining complete.")
        ...
        logger.info(f'\nStarting Rendering~')
        if lp.eval:
            aerial_visible_count, street_visible_count = render_sets(
                lp, op, pp, -1, skip_train=True, skip_test=False, 
                wandb=wandb_run, logger=logger
            )
        else:
            aerial_visible_count, street_visible_count = render_sets(
                lp, op, pp, -1, skip_train=False, skip_test=True, 
                wandb=wandb_run, logger=logger
            )
        logger.info("\nRendering complete.")
        ...
        evaluate(
            lp.model_path, eval_name, 
            aerial_visible_count=aerial_visible_count, 
            street_visible_count=street_visible_count, 
            wandb=wandb_run, logger=logger
        )
        logger.info("\nEvaluating complete.")
