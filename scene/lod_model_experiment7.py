...
class GaussianLoDModel(BasicModel):
    ...
    def prune_anchor(self, mask):
        ...
        # ================= [新增代码 START] =================
        # 同步剪枝 anchor_source_mask
        if hasattr(self, "anchor_source_mask") and self.anchor_source_mask is not None:
            # 确保 mask 在正确的设备上
            if self.anchor_source_mask.device != valid_points_mask.device:
                self.anchor_source_mask = self.anchor_source_mask.to(valid_points_mask.device)
            self.anchor_source_mask = self.anchor_source_mask[valid_points_mask]
        # ================= [新增代码 END] =================
    ...
    def anchor_growing(self, grads, opt, offset_mask, iteration):
        ...
        for cur_level in range(self.street_levels):
            ...
            if candidate_anchor.shape[0] > 0:
                ...
                # ================= [新增代码 START: 处理 Mask 继承] =================
                new_source_mask = None
                if hasattr(self, "anchor_source_mask") and self.anchor_source_mask is not None:
                    # 1. 将 mask 扩展到 offsets 维度: [N] -> [N, k]
                    # 这里的 self.n_offsets 应该和 feat 扩展时的 k 一致
                    expanded_source = self.anchor_source_mask.unsqueeze(1).repeat(1, self.n_offsets).view(-1)
                    
                    # 2. 如果 candidate_mask 因为 length_inc 变长了，mask 也需要对齐
                    # candidate_mask 的长度是 (N + length_inc) * k
                    # expanded_source 的长度是 N * k
                    # 不过通常 anchor_growing 是在 backward 之后，update 之前，N 应该是当前 anchor 数
                    # 上面代码 checks: if length_inc > 0: candidate_mask cat zeros...
                    # 所以我们需要把 expanded_source 补齐
                    
                    total_candidates_len = candidate_mask.shape[0]
                    current_source_len = expanded_source.shape[0]
                    if total_candidates_len > current_source_len:
                        padding = torch.ones(total_candidates_len - current_source_len, dtype=expanded_source.dtype, device=expanded_source.device) * 2 # 默认为共享(2)或其他? 或者0?
                        # 实际上 length_inc > 0 说明有新点还没 mask，暂时设为 2 (Shared) 比较安全
                        expanded_source = torch.cat([expanded_source, padding], dim=0)
                    
                    # 3. 选出候选点的 mask
                    candidate_source = expanded_source[candidate_mask].float() # 转 float 方便 scatter
                    
                    # 4. 处理重复点 (voxel grid 合并)
                    # 我们使用 scatter_max 来决定合并后的 mask 值 (假设 2>1>0, 优先保留共享或街景)
                    # inverse_indices: [M_candidates] -> [U_unique]
                    # unique_source: [U_unique]
                    unique_source, _ = scatter_max(candidate_source, inverse_indices, dim=0)
                    
                    # 5. 应用 remove_duplicates (weed_out)
                    final_new_source = unique_source[remove_duplicates]
                    
                    # 6. 转回原有类型
                    new_source_mask = final_new_source.to(self.anchor_source_mask.dtype)
                # ================= [新增代码 END] =================
                ...
                # ================= [新增代码 START: 拼接 Mask] =================
                if new_source_mask is not None:
                    self.anchor_source_mask = torch.cat([self.anchor_source_mask, new_source_mask], dim=0)
                # ================= [新增代码 END] =================
