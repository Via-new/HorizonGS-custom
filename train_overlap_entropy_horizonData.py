...
def training(dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, wandb=None, logger=None, ply_path=None):
    ...
    # --- [修改点 1] 移除了原先手动的相机分类 for 循环 ---
    # 假设你的 Scene 类加载器内部已经正确处理了 image_type
    
    # --- [保留] 初始化 Level 0 重叠管理器 (HorizonGS 特有逻辑) ---
    l0_manager = Level0OverlapManager()
    
    # 为了防止因为移除了手动分类导致 scene 属性缺失，这里做一个安全检查
    # 如果 Scene 加载器没有设置 add_aerial/add_street，我们尝试从相机列表推断
    has_aerial = getattr(scene, 'add_aerial', False)
    has_street = getattr(scene, 'add_street', False)
    
    if not has_aerial or not has_street:
        # 备用检查：扫描一遍相机列表看看有没有对应的类型
        n_aerial = len([c for c in scene.getTrainCameras() if c.image_type == "aerial"])
        n_street = len([c for c in scene.getTrainCameras() if c.image_type == "street"])
        if n_aerial > 0 and n_street > 0:
            has_aerial = True
            has_street = True

    if has_aerial and has_street:
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if pipe.camera_balance:
            ...
            # 解析比例 (例如 "1-1" 或 "2-1")
            if hasattr(pipe, 'camera_proportion'):
        ...
        with torch.no_grad():
            ...
            if iteration % pipe.vis_step == 0 or iteration == 1:
                ...
                if l0_manager.is_initialized and iteration > ENTROPY_START_ITER:
                    ...
                    combined = torch.cat([image, mask_vis], dim=2) 
